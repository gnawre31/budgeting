...
export default function ReimbursementBalance() {
    ...
    const fetchData = useCallback(async () => {
        ...
        // Fetch own reimbursements AND partner's reimbursements where this user has a share
        const { data, error } = await supabase
            .from("transactions")
            .select("id, merchant, amount, partner_amount, user_id, date")
            .eq("category", "Reimbursement")
            .is("parent_id", null)
            .or(`user_id.eq.${user.id},and(partner_id.eq.${user.id},partner_amount.gt.0)`)
            .order("date", { ascending: false });

        if (error) { console.error("ReimbursementBalance fetch error:", error); setLoading(false); return; }

        // Normalise: for own transactions the outstanding amount is `amount`;
        // for partner-posted transactions it's `partner_amount` (this user's share).
    ...
    const visible  = overflow > 0 ? items.slice(0, MAX_VISIBLE) : items;
    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-200/60 shadow-sm relative overflow-hidden flex flex-col h-full max-h-[360px]">
            {loading && (
            ...
            <div className="px-4 sm:px-6 py-5 border-b border-gray-100 flex items-start justify-between">
                ...
                {items.length > 0 && (
                    <Link
                        to="/reconcile"
                        className="text-sm font-medium text-indigo-500 hover:text-indigo-600 transition-colors px-3 py-1.5 border border-indigo-100 rounded-xl hover:bg-indigo-50 whitespace-nowrap mt-0.5"
                    >
                        Reconcile →
                    </Link>
                )}
            ...
            <div className="px-4 sm:px-6 py-5 flex-1 flex flex-col">
                    ...
                    <>
                        ...
                        <div className="divide-y divide-gray-50">
                            ...
                            {overflow > 0 && (
                                <Link
                                    to="/reconcile"
                                    className="flex items-center justify-center py-2.5 text-xs font-medium text-gray-400 hover:text-indigo-500 transition-colors"
                                >
