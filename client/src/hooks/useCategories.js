...
export function useCategories() {
        ...
        try {
            ...
            try {
                ...
                if (cats.length === 0) {
                    ...
                    // Also seed for partner if they have none yet
                    if (pid) {
    ...
    // ── Derived lists (memoized so array references are stable across renders) ─
    ...
    const addCategory = useCallback(async (name, type, flags = {}) => {
        ...
        if (partnerId) {
            try {
                await supabase
                    .from("user_categories")
                    .upsert({ user_id: partnerId, name: name.trim(), type, is_system: false, ...flags },
